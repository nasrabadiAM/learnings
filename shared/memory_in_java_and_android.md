...
---
جاوا از gc  برای مدیریت مموری استفاده می‌کند. به این صورت کار می‌کند که ابتدا کل آبجکت‌های هیپ‌مموری را پیمایش می‌کند و هر آبجکتی که رفرنسی به آن وجود نداشته باشد و یا احتیاجی به آن نباشد را علامت می‌زند. در مرحله دوم این آبجکت ها را پاک می‌کند و در مرحله سوم مموری را مرتب می‌کند و جاهای خالی را پر می‌کند. 

زمانی که gc  آبجکت‌های بی‌مصرف که دیگر نیازی به آن‌ها نیست را به عنوان آبجکتی که به آن نیاز است در نظر بگیرد و آن را از حاظه پاک نکند، مموری لیک رخ داده است.

مموری‌لیک باعث می‌شود فضای با ارزش مموری غیر‌قابل استفاده باشد. اگر این امر ادامه پیدا کند و مموری پر شود، اندروید مکررا شروع به اجرای gc  می‌کند تا بتواند مموری مورد نیاز را تامین کند. gc  کاری سنگین است و هر بار که اجرا می‌شود، تمام اپ‌ها و ترد‌ها به صورت موقت و تا پایان آن pause می‌شوند. برای همین اپ کند شده و در نهایت پیام ANR  را به کاربر نشان می‌دهد.



چطور مموری‌لیک را پیدا و حل کنیم؟
---
برای پیدا کردن مموری‌لیک چند ابزار وجود دارد:

۱-استفاده از leakcanary

۲-استفاده از Android Profiler  اندروید استودیو

۳-گرفتن دامپ مموری به صورت دستی و مقایسه دامپ قبل و بعد از gc 



چگونه با استفاده از adb دامپ مموری را بگیریم؟
---
با استفاده از دستور dumpsys   در adb می‌توانیم، دامپ مموری را بگیریم.
