...
<p dir="rtl"><b>
برخی دیگر از ابزارها:
</b></p>

<p dir="rtl">
همانطور که صحبت کردیم، یک سری متد هم در کلاس آبجکت وجود دارد که می‌توانند در 
  Threading
  به ما کمک کنند.
  این متد‌ها عبارت‌اند از 
...
صدا زدن 
  wait() 
  باعث می‌شود تا ترد فعلی تا زمانی که 
  notify()
  یا 
  notifyAll()
  صدا زده شود، متوقف شود و یا زمانی که 
  Thread.intrupt() 
  صدا زده شود، اکسپشن بدهد.
  البته 
  notify()
  و 
  notifyAll()
  باید از تردی دیگر صدا زده شود(ترد فعلی متوقف شده :دی)
بهتر است 
  notify()
  را صدا نزنیم، مگر اینکه واقعا بدانیم داریم چه کاری می‌کنیم.
  همچنین استفاده بیش از اندازه از 
  wait()
  و 
  notify()
  کدها را پیچیده می‌کند. بهتر است زیاد استفاده نکنیم.
</p>


مشکل race condition در مالتی‌تردینگ چیست؟
---
در دنیای multi-threading علاوه برمزایای زیادی که وجود دارد، گاها اگر درست از آن‌ها استفاده نکنیم مشکلاتی هم به وجود خواهد آمد.
یکی از این مشکلات می‌تواند race condition باشد. این مشکل زمانی به وجود می‌آید که دو ترد به صورت همزمان بخواهند به یک ریسورس دسترسی داشته باشند آن را تغییر دهند. اینجاست که ممکن است مشکلاتی به وجود بیاید.
...
در مثال بالا اگر تردی دیگر بعد از if مقدار را تغییر دهد، جواب پایانی اشتباه خواهد بود.

برای برطرف کردن این مشکل قبل از چک کردن مقدار lock ای را روی آبجکت می‌گیریم و بعد از آنکه کارمان با آبجکت تمام شد، لاک را آزاد می‌کنیم.
...
راه دیگری که می‌توان رفت، این است که از synchronized استفاده کنیم به این صورت که بلاک کدی که می‌خواهیم را داخل بلاک synchronized قرار می‌دهیم.


در هر حال mutable Shared data/resource ها منبع ایجاد باگ در سیستم‌های مالتی‌ترد است و باید از آن‌ها کمتر استفاده کنیم.
